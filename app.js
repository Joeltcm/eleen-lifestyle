  ...
  const pintar = async () => {
    ...
    try {
      ...
      destino.innerHTML = reglas.length ? `<div class="gasto-lista">${reglas.map(regla => {
        ...
        return `<article class="gasto-item">
          <div><b>${escapeHtml(regla.full_name)}${regla.paused ? ' · Paquete en pausa' : ''}</b><small>${dias} · ${hora} · ${regla.duration_minutes} min${regla.routine_title ? ` · ${escapeHtml(regla.routine_title)}` : ''}<br>${regla.proximas} sesion${regla.proximas === 1 ? '' : 'es'} ya agendada${regla.proximas === 1 ? '' : 's'}${regla.ends_on ? ` · hasta ${dateOnly(regla.ends_on)}` : ' · sin fecha de fin'}${regla.paused ? ' · las sesiones futuras están retenidas' : ''}</small></div>
